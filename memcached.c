...
#ifdef USE_REPLICATION
    APPEND_STAT("replication ", "%s", is_master ? "MASTER" : "SLAVE");
    APPEND_STAT("repcached_version", "%s", REPCACHED_VERSION);
    APPEND_STAT("repcached_qi_free", "%u", settings.rep_qmax - get_qi_count());
    APPEND_STAT("repcached_wdata", "%u", rep_conn ? rep_conn->wbytes + (rep_conn->wcurr - rep_conn->wbuf) : 0);
    APPEND_STAT("repcached_wsize", "%u", rep_conn ? rep_conn->wsize : 0);
#endif /*USE_REPLICATION*/
    STATS_UNLOCK();
}
...
static void drive_machine(conn *c) {
    ...
    if(rep_exit && (c->state != conn_pipe_recv)){
		fprintf(stderr, "rep_exit replication_send\n");
    ...
    while (!stop) {
        ...
        case conn_parse_cmd :
#ifdef USE_REPLICATION
			if(rep_conn == c && !rep_start) {
				while(c->rbytes > 0) {
					int res = try_read_command(c);
					if(res == 0) {
						break;
					}
					if(c->rbytes == 0) {
...
int main (int argc, char **argv) {
    ...
    while (-1 != (c = getopt(argc, argv,
          ...
          "X:"  /* Extended generic options */
          "x:"  /* Extended generic options */
          "q:"  /* Extended generic options */
        ))) {
#else
    /* process arguments */
    while (-1 != (c = getopt(argc, argv,
...
static int replication_start()
{
    ...
    if (settings.socketpath == NULL) {
        int udp_port;
        if (server_sockets(settings.port, tcp_transport, NULL)) {
            fprintf(stderr, "failed to listen\n");
            return(-1);
        }
        udp_port = settings.udpport ? settings.udpport : settings.port;
        if (server_sockets(udp_port, udp_transport, NULL)) {
...
static int replication_server_init()
{
    ...
    if(server_socket_replication(settings.rep_listen_port)){
        fprintf(stderr, "replication: failed to initialize replication server socket %d \n",settings.rep_listen_port);
        return(-1);
    }
    if (settings.verbose > 0)
        fprintf(stderr, "replication: listen port %d\n",settings.rep_listen_port);
	is_master = true;
...
static int replication_connect()
{
    ...
    if(pipe(p) == -1){
        ...
        rep_send = conn_new(p[1], conn_pipe_send, EV_WRITE | EV_PERSIST, DATA_BUFFER_SIZE, tcp_transport, main_base);
...
static int replication_close()
{
    ...
    if(rep_recv){
        ...
        rep_recv = NULL;
        if (settings.verbose > 1) {
            fprintf(stderr, ">%d: replication: qitem free %d items\n", qi_free_list(), sfd);
            fprintf(stderr, ">%d: replication: close recv %d items\n", c, sfd);
        }
    }
    if(rep_send){
		int sfd = rep_send->sfd;
        conn_close(rep_send);
        rep_send = NULL;
        if (settings.verbose > 1)
            fprintf(stderr,">%d: replication: close send\n", sfd);
    }
    if(rep_conn){
		int sfd = rep_conn->sfd;
        ...
        rep_conn = NULL;
        if (settings.verbose > 1)
            fprintf(stderr,">%d: replication: close conn\n", sfd);
    }
    if(!rep_exit)
...
static int replication_send(conn *c)
{
    ...
    while(c->wbytes){
        ...
        if(w == -1){
            ...
            fprintf(stderr, "<%d: replication: send error: %s\n", c->sfd, strerror(errno));
            replication_close();
            return(-1);
        }
        c->wbytes -= w;
