...
static int replication_rep(conn *c, item *it)
{
    ...
    len += 1;
    len += replication_get_num(NULL, it->data->cas);
    len += strlen(n);
    len += it->nbytes;
    len += strlen(n);
    ...
    *(p++) = ' ';
    p += replication_get_num(p, it->data->cas);
    memcpy(p, n, strlen(n));
    p += strlen(n);
...
{
    item *it;
    uint32_t hv = q->key ? hash(q->key, strlen(q->key)) : 0;
    switch (q->type) {
    case REPLICATION_REP:
        if(it = assoc_find(q->key, strlen(q->key), hv))
